...
void lcd_display_dir(uint8_t dir)
{
    ...
    if (lcddev.dir == 0)         /* 竖屏 */
    {
        lcddev.width = lcddev.pheight;
        lcddev.height = lcddev.pwidth;
        esp_lcd_panel_swap_xy(panel_handle, false);
        esp_lcd_panel_mirror(panel_handle, false, false);
    }
    else if (lcddev.dir == 1)    /* 横屏 */
    {
        lcddev.width = lcddev.pwidth;
        lcddev.height = lcddev.pheight;
...
void lcd_draw_circle(uint16_t x0, uint16_t y0, uint16_t r, uint16_t color)
{
    ...
    uint32_t xr = r;
    uint16_t color_tmp = ((color & 0x00FF) << 8) | ((color & 0xFF00) >> 8);   /* 需要转换一下颜色值 */
    lcd_draw_hline(x0 - r, y0, 2 * r, color_tmp);
    ...
    for (i = 1; i <= imax; i++)
    {
        if ((i * i + xr * xr) > sqmax)
        {
            ...
            if (xr > imax)
            {
                lcd_draw_hline (x0 - i + 1, y0 + xr, 2 * (i - 1), color_tmp);
                lcd_draw_hline (x0 - i + 1, y0 - xr, 2 * (i - 1), color_tmp);
            }
        ...
        lcd_draw_hline(x0 - xr, y0 + i, 2 * xr, color_tmp);
        lcd_draw_hline(x0 - xr, y0 - i, 2 * xr, color_tmp);
    }
}
