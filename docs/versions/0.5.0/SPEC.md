...
做 UTF-8 安全省略；空值或无 `generic-source` 的兼容项显示 `Agent`。侧栏使用状态和任务标题
短句区分会话，不显示四位 HMAC 片段；假名 ID 仅用于内部选择和协议身份。
主屏有侧栏时，计时仍与无侧栏布局一样在标题行右对齐；右侧距离侧栏 8 px，状态与计时至少
保留 6 px 间距，不得把计时挤到图标下方。

底栏的隐藏待处理计数改为条件显示，不再固定输出 `!N`：

- `hidden_attention == 0` 时完全省略提示，例如只显示 `最近会话 2/2`；
- `hidden_attention > 0` 时显示明确文字 `待处理 N`，例如 `最近会话 2/5 / 待处理 3`；
...
运行态连续 120 秒没有 hook 时先标记为过期并冻结计时；再保留 15 分钟仍无新事件时退出
“最近会话”投影。Host 继续保留有界内部记录，因此额度耗尽、客户端休眠或 hook 暂停后的迟到
事件仍能恢复该会话。失败态保留 10 分钟后自动移除；等待输入和等待批准不适用自动清理规则。
...
- 第一版内置 `saki_stage` 与无图片的 `classic` 回退包。`saki_stage` 为九种状态各提供一张
  48×48 RGB565A8 静态图；ActivityKind 暂时继承所属状态图片，后续素材包可以增加覆盖，但
  不得把 Agent 产品名带入映射层。
...
- 提供“关闭 / 轻量”本地设置并持久化；0.5 默认使用 `saki_stage` + “轻量”。K1 短按切换
  文案模式，按住 K1 2–5 秒后松开切换素材包；K2 的 BLE 手势保持不变。
...
- 不修改 Host 32 项、设备 4 项、注意优先级或来源身份隔离语义。
