...
- 主屏允许完整有界 label；侧栏按固定像素宽度显示状态和标题短句，不暴露 HMAC ID 片段；
...
运行态 stale 后保留 15 分钟观察窗口，随后仅从 display projection 隐去，内部有界记录仍可被
迟到的 tool/stop/resume 事件恢复。新会话触发容量压力时，已隐去 stale 记录属于优先淘汰项；
投影 `total` 只计算仍在“最近会话”中的记录。失败态使用 10 分钟 TTL 并把剩余时间写入既有
checkpoint 的 `expires_in`；旧 checkpoint 中无 TTL 的失败态在恢复时迁入同一保留窗口。

紧凑主卡不改变计时的视觉语义：以侧栏左边缘前 8 px 为内容右边界，计时使用 56 px 右对齐，
与状态标签保留 6 px 间距。状态空间不足时由状态标签省略，计时不换行或移到图标下方。
...
新增 `saki_theme` 固件组件，定义稳定 pack ID、显示名、九状态图标和逐状态文案池；UI 只通过
注册表 API 取得当前 pack，不直接引用某一套生成符号。内置 `saki_stage` 和 `classic`，后者不含
图片与趣味文案，是资源缺失和用户偏好的完整回退。增加素材包只注册新的只读 pack，不能修改
协议、Agent adapter 或 SessionRegistry，也不允许运行时下载或加载代码。

`saki_stage` 的原始 3×3 RGBA sheet、生成提示、来源哈希和声明与包同目录保存。素材表允许外侧
留白和三列/三行宽度不均，但格间必须有完整透明沟槽。固定脚本先以 alpha >= 8 的投影自动检测
两条横向和纵向沟槽，不使用画布三等分；随后按每格可见边界裁切、等比缩放至 48×48，并生成
LVGL RGB565A8 C 资源。1536×1024 RGBA 原图是素材包高清母版，固件派生物不得反向覆盖母版；
后续尺寸一律从母版重新生成。缺少沟槽、格子过窄或输入 SHA-256 不匹配时拒绝生成。
生成文件加显式说明，不手工修改。九张运行时图片共 62,208 B；源 PNG 仅用于复现，不嵌入
固件。映射层只接受 `AgentState`，ActivityKind 在 0.5 继承当前状态图片，不接受
Codex/Claude source。
...
透明状态图后由 UI 绘制 52×52 圆角状态底板：深色底面混入约 30% 当前状态色，边框和轻微
辉光使用完整状态色。底板与状态图共用转场 key，仅在 state、焦点 task/run 或素材包变化时
淡入；`classic`、离线或资源缺失时隐藏底板并回退原状态圆点。配色属于语义 UI 层，不烘焙进
素材 PNG，因此未来素材包默认继承，也可以在后续规格中增加受控的 palette override。

低内存和资源失败路径必须回到可读界面。不得在运行中下载、解码无界图片或从 Host 接收素材。

## 8. 趣味文案设置

文案库随素材包按状态组织。选择使用稳定 FNV-1a hash，输入为 pack ID、假名 task ID、run ID、
state 和 activity key；状态不变时不得重新抽取，也不保存任务文本或原生身份。真实摘要或等待/
失败 detail 始终占第一行，趣味文案只占第二行；详情页、离线和 BLE 覆盖层不显示趣味文案。

默认选中 `saki_stage` 和“轻量”文案。K1 短按在“轻量/关闭”间切换，按住 K1 2–5 秒后松开
切换注册表中的下一个素材包；设置写入独立 `saki_theme` NVS namespace。K2 配对/重连/清除、
亮度唤醒和详情触摸均不变。按住 K1 超过 5 秒不执行切换，避免误触复用的长按阈值。
